...
-- Item Slots
local ItemSlots = { e_spell_slot.item1, e_spell_slot.item2, e_spell_slot.item3, e_spell_slot.item4, e_spell_slot.item5, e_spell_slot.item6, e_spell_slot.item7 }
...
local get_buff_data = function(unit, buffname)
	...
	return {active = false, hard_cc = false, disabling = false, knock_up = false, silence = false, cripple = false, invincible = false, slow = false, type = 0, start_time = 0, end_time = 0, alt_amount = 0, name = "", amount = 0}
end
...
local DMG_REDUCTION_BUFFS = {
	["Annie"] = {buff = "AnnieE", amount = function(target) return 1 - ({0.10,0.13,0.16,0.19,0.22})[target:get_spell_book():get_spell_slot(e_spell_slot.e).level] end}, -- TODO: find right values.
	["Alistar"] = {buff = "FerociousHowl", amount = function(target) return 1 - ({0.55, 0.65, 0.75})[target:get_spell_book():get_spell_slot(e_spell_slot.r).level] end},
	["Belveth"] = {buff = "BelvethE", amount = function(target) return 1 - 0.7 end},
	["Braum"] = {buff = "BraumShieldRaise", amount = function(target) return 1 - ({0.3, 0.325, 0.35, 0.375, 0.4})[target:get_spell_book():get_spell_slot(e_spell_slot.e).level] end},
	["Galio"] = {buff = "galiowpassivedefense", DamageType = 2, amount = function(target) return 1 - ({0.2, 0.25, 0.30, 0.35, 0.40})[target:get_spell_book():get_spell_slot(e_spell_slot.w).level] + (0.05 * target:get_ability_power() / 100) + (0.8 * target.bonus_mr / 100) end},
	["Galio"] = {buff = "galiowpassivedefense", DamageType = 1, amount = function(target) return 1 - ({0.1, 0.125, 0.15, 0.175, 0.20})[target:get_spell_book():get_spell_slot(e_spell_slot.w).level] + (0.025 * target:get_ability_power() / 100) + (0.4 * target.bonus_mr / 100) end},
	["Garen"] = {buff = "GarenW", amount = function(target) return 1 - 0.3 end},
	["Gragas"] = {buff = "GragasWSelf", amount = function(target) return 1 - ({0.1, 0.12, 0.14, 0.16, 0.18})[target:get_spell_book():get_spell_slot(e_spell_slot.w).level] + 0.04 * target:get_ability_power() / 100 end},
	["KSante"] = {buff = "KSanteW", amount = function(source, target) return 1 - 0.25 + (0.10 * math.floor(target.bonus_armor/100)) + (0.10 * math.floor(target.bonus_mr/100)) + (0.10 * math.floor((target.max_health - get_base_hp(target))/100)) end},
	["Malzahar"] = {buff = "malzaharpassiveshield", amount = function(target) return 1 - 0.9 end},
	["MasterYi"] = {buff = "Meditate", amount = function(source, target) return 1 - ({0.45, 0.475, 0.50, 0.525, 0.55})[target:get_spell_book():get_spell_slot(e_spell_slot.w).level] / (source:is_building() and 2 or 1) end},
  ...
  local DMG_REDUCTION_ITEMS = {
	...
	["ForceOfNature"] = {buff = "4401maxstacked", IsAA = false, DamageType = 2,  amount = function(source, target, DamageType, amount) -- Force of Nature // 4401
...
function PENETRATION_ITEM:new(lethality, armorPenetration, magicPenetration, extraLethality, extraArmorPenetration, extraMagicPenetration)
    local newItem = {
  ...
  local DMG_MULTIPLIERS_BUFFS = { -- format: lethality (int), armor penetration (percent)
...
local SPECIAL_AA = {
	...
	["TwistedFate"] = function(args) --TwistedFateW
	  if has_buff(args.source, "cardmasterstackparticle") then
		args.RawMagical = args.RawMagical + 30 + 25 * args.source:get_spell_book():get_spell_slot(e_spell_slot.e).level + 0.5 * args.source:get_ability_power()
	  end
	  if has_buff(args.source, "BlueCardPreAttack") then
		args.DamageType = DAMAGE_TYPE_MAGICAL
		args.RawMagical = args.RawMagical + 20 + 20 * args.source:get_spell_book():get_spell_slot(e_spell_slot.w).level + 0.5 * args.source:get_ability_power()
	  elseif has_buff(args.source, "RedCardPreAttack") then
		args.DamageType = DAMAGE_TYPE_MAGICAL
		args.RawMagical = args.RawMagical + 15 + 15 * args.source:get_spell_book():get_spell_slot(e_spell_slot.w).level + 0.5 * args.source:get_ability_power()
	  elseif has_buff(args.source, "GoldCardPreAttack") then
		args.DamageType = DAMAGE_TYPE_MAGICAL
		args.RawMagical = args.RawMagical + 7.5 + 7.5 * args.source:get_spell_book():get_spell_slot(e_spell_slot.w).level + 0.5 * args.source:get_ability_power()
	  end
	end,
	...
	["Udyr"] = function(args) --Udyr Q and R
	  if has_buff(args.source, "UdyrPAttackReady") then
		...
		if has_buff(args.source, "UdyrQ") then
		  ...
		  args.RawPhysical = args.RawPhysical + ({ 5, 13, 21, 29, 37, 45 })[qlevel] + (0.20 * args.source:get_bonus_attack_damage()) +
			  (
		...
		elseif has_buff(args.source, "UdyrR") then
		  args.DamageType = DAMAGE_TYPE_MAGICAL
		  args.RawMagical = args.RawMagical + (10 + 20 / 17 * (level - 1)) + (0.30 * args.source:get_ability_power())
		end
	  end
	end,
	...
	["Vayne"] = function(args) --VayneQ
	  if has_buff(args.source, "vaynetumblebonus") then
		args.RawPhysical = args.RawPhysical
			+ (0.25 + 0.05 * args.source:get_spell_book():get_spell_slot(e_spell_slot.q).level) * args.source.total_damage
	  end
	end,
	...
	["Zac"] = function(args) --ZacQ
	  local level = args.source:get_spell_book():get_spell_slot(e_spell_slot.q).level
	  args.DamageType = DAMAGE_TYPE_MAGICAL
	  args.RawMagical = ({ 40, 55, 70, 85, 100 })[level] + (0.3 * args.source:get_ability_power()) + (0.025 * args.source.max_health)
	end,
...
local ITEM_DAMAGE = {
	...
	["LichBane"] = function(args) --"Lich Bane"
	  if has_buff(args.source, "lichbane") > 0 then
		args.RawMagical = args.RawMagical + 0.75 * args.source:get_attack_damage() + 0.5 * args.source:get_ability_power()
	  end
	end,
		...
		if target:GetBuff(i).count > 0 then
		  ...
		  if DMG_REDUCTION_BUFFS[target.champion_name] then
			if buff.name == DMG_REDUCTION_BUFFS[target.champion_name].buff and (not DMG_REDUCTION_BUFFS[target.champion_name].DamageType or DMG_REDUCTION_BUFFS[target.champion_name].DamageType == DamageType) then
			  amount = amount * DMG_REDUCTION_BUFFS[target.champion_name].amount(source, target, DamageType, amount)
			end
		  end
		end
	  end
...
DB.SpellDB = {
	...
	["Jinx"] = { -- Last updated on 26.03.2023, patch 13.6 h2
	  {Slot = "Q", Stage = 1, DamageType = 1, Damage = function(source, target, level) return 0.1 * source:get_attack_damage() end},
	  {Slot = "W", Stage = 1, DamageType = 1, Damage = function(source, target, level) return ({10, 60, 110, 160, 210})[level] + 1.6 * source:get_attack_damage() end}, -- 10 / 60 / 110 / 160 / 210 (+ 160% AD)
	  {Slot = "E", Stage = 1, DamageType = 2, Damage = function(source, target, level) return ({70, 120, 170, 220, 270})[level] + source:get_ability_power() end}, -- 70 / 120 / 170 / 220 / 270 (+ 100% AP)
	  {Slot = "R", Stage = 1, DamageType = 1, Damage = function(source, target, level) local dmg = (({30, 45, 60})[level] + (0.15 * source:get_bonus_attack_damage()) * (1.10 + (0.06 * math.min(math.floor(source.position:dist_to(target.position) / 100),15)))) + (({25, 30, 35})[level] / 100 * get_missing_hp(target)); return dmg end}, -- Base damage
	  {Slot = "R", Stage = 2, DamageType = 1, Damage = function(source, target, level) local dmg = (({24, 36, 48})[level] + (0.12 * source:get_bonus_attack_damage()) * (1.10 + (0.06 * math.min(math.floor(source.position:dist_to(target.position) / 100),15)))) + (({20, 24, 28})[level] / 100 * get_missing_hp(target)); if target:is_ai() then return math.min(1200, dmg) end; return dmg end}, -- Splash damage TODO: test if is_ai is the correct function to use.
	},
...
local get_crit_percent = function(source)
	...
	elseif source.charName == "Fiora" then
	  baseCriticalDamage = ({1.60, 1.70, 1.80, 1.90, 2.00})[source:get_spell_book():get_spell_slot(e_spell_slot.w).level]
	...
	local modCrit =  baseCriticalDamage + (((Item:HasItem(source, "InfinityEdge")) and (source.critChance >= 0.40) and 0.35) or 0) --TODO:
...
X.helper.calculate_damage = function(source, target, DamageType, amount, IsAA)
	...
	if target:is_hero() then
	  ...
	  elseif target.champion_name == "Leona" and get_buff_data(target, "LeonaSolarBarrier") then
		flatPassive = flatPassive - (({8, 12, 16, 20, 24})[target:get_spell_book():get_spell_slot(e_spell_slot.w).level or 1])
	  elseif target.champion_name == "Amumu" and get_buff_data(target, "Tantrum") then
		flatPassive = flatPassive - (({5, 7, 9, 11, 13})[target:get_spell_book():get_spell_slot(e_spell_slot.e).level or 1] + (0.03 * target.bonus_mr) + (0.03 * target.bonus_armor)) --TODO: max 50%
	  end
...
X.helper.get_spell_damage = function(spell, target, source, stage, level)
	...
	if spell == "Q" or spell == "W" or spell == "E" or spell == "R" or spell == "QM" or spell == "WM" or spell == "EM" and source:is_hero() then
	  local level = level or source:get_spell_book():get_spell_slot(({["Q"] = e_spell_slot.q, ["QM"] = e_spell_slot.q, ["W"] = e_spell_slot.w, ["WM"] = e_spell_slot.w, ["E"] = e_spell_slot.e, ["EM"] = e_spell_slot.e, ["R"] = e_spell_slot.r})[spell]).level
	  ...
	  if DB.SpellDB[source.champion_name] then
		...
		for v = #dmgtable, 1, -1 do
		  ...
		  if spells.Stage == stage then
			return X.helper.calculate_damage(source, target, spells.DamageType, spells.Damage(source, target, level), false)
		  end
		end
	  end
	end
